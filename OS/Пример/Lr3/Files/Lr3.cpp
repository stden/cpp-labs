	...
	while (!EndOfProgram) {
		...
		while (!EndOfEnter) {  
			...
			switch (ch) {      // В зависимости от символа выполняем следующие действия 
				case 10 : EndOfEnter = 1; s[i] = 0; break;         // Нажали Enter, завершаем ввод
				case 27 : EndOfEnter = 1; EndOfProgram = 1; break; // Нажали Escape, выходим из программы
				default : s[i] = ch; i++; break;                   // Нажали любую другую клавишу, записываем ее в строку s
			} // switch (ch)	
		} // while (!EndOfEnter)
				...
				switch (operation_id) {
					...
					case '1' : 
						...
						if (dwFileAttributes == INVALID_FILE_ATTRIBUTES) { 
							printf("An error occurred. Invalid file attributes"); getch(); }
						else {
					...
					// Выбираем новый файл
					case '5' : CloseHandle(hFile); EndOfWork =1; break;


					// Выходим из программы
					case '6' : CloseHandle(hFile); EndOfWork = 1; EndOfProgram = 1; printf("\n\n "); break;

					// Выбрали неверную операцию
					default  : printf("Wrong id operation!"); getch(); break;
				}
			} // while !EndOfWork
		}
		else 
			{printf(" The file is not found, try again!"); getch();}
