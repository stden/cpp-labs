...
hFile = CreateFile(str,  // file name 
    ...
    m_res+="Could not create file."; // process error 
		   ...
		   {m_res="File  ";m_res+=str; m_res+="   Have been created succesfully" ;}
}
...
result=GetFileInformationByHandle(		// ????????? ?????????? ? ????? ?? ??????
    ...
    if(lpFileInformation->dwFileAttributes ==32)// &FILE_ATTRIBUTE_ARCHIVE)
	   ...
	   if(lpFileInformation->dwFileAttributes ==33)//&FILE_ATTRIBUTE_ARCHIVE) &&lpFileInformation->dwFileAttributes ==FILE_ATTRIBUTE_READONLY)
	   {sprintf(str,"File attributes are FILE ATTRIBUTE ARCHIVE AND READONLY \r\n",lpFileInformation->dwFileAttributes);  
	   m_res+=str;}
	   else 
	   if(lpFileInformation->dwFileAttributes == 34)//FILE_ATTRIBUTE_ARCHIVE &&lpFileInformation->dwFileAttributes ==FILE_ATTRIBUTE_HIDDEN)
	   {sprintf(str,"lpFileInformation->dwFileAttributes FILE_ATTRIBUTE_HIDDEN AND ARCHIVE \r\n",lpFileInformation->dwFileAttributes);  
	   m_res+=str;}     
         else
        	   if(lpFileInformation->dwFileAttributes ==3)// &FILE_ATTRIBUTE_HIDDEN) && (lpFileInformation->dwFileAttributes &FILE_ATTRIBUTE_READONLY))
	   {sprintf(str,"A read-only file FILE_ATTRIBUTE_READONLY  AND HIDDEN \r\n",lpFileInformation->dwFileAttributes);  
	    m_res+=str;}     
	    	else
       if(lpFileInformation->dwFileAttributes==35 )
       ...
       if(lpFileInformation->dwFileAttributes ==2)//&FILE_ATTRIBUTE_HIDDEN)
...
if(Input.DoModal()==IDOK)
    ...
    m_res+="Could not Get type of this file."; // process error 
	 ...
	 {m_res="You didn't select attribute"; test=FALSE;Input.DoModal();}
