...
//---------------------------------------------------------------------------
void main() {
  int N(9);           // Размер буфера
  char s[MAX_PATH];    // Буфер

  // Выдача текущей директории  - возвращает число символов записанное
  // в буфер (длина то бишь)
  ...
  h = CreateFile( TEXT(s),          // Имя
                  ...
                  CREATE_ALWAYS,    // Создается файл, если есть уже
                  // такой, то перезаписывается
  ...
  if (h == INVALID_HANDLE_VALUE)
    ...
    cout <<    New file was created successfully\n\n";
       ...
       cout << "6. Enter the name of copying file        : ";
         ...
         char d[80];
         cout << "
       Enter the name of copy of copyng file : ";
         cin  >> d; // Копия файла
       ...
       cout << "7. Enter the name of moving file     : ";
         ...
         cout << "
       Enter the new name of moving file : ";
         cin  >> d;
         if (!MoveFile(s,d))
         // Вывод ошибки в случае неудачи
         if (GetLastError() == ERROR_FILE_EXISTS)
