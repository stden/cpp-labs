...
class Transfer {
 public:
  ...
  States    states;
  Transfer() {}
  Transfer( Terminals terminals, States states ) {
    this->states = states;
...
class Parser {
 public:
  ...
  void    AttachFile(const char* fileName) {
    lexer->AttachFile(fileName);
  }
  string*   GetTerminalsString() const  {
    return terminals;
  }
  States*   GetStatesVector() const   {
    return states;
  }
  Transfers*  GetTransfersTable() const {
    return transfers;
  }
...
bool Parser::ProcessNewBlock() {
  ...
  if ( w->type != ID ) throw ParserException( "block specifier expected after '<'" );
    ...
    string e("invalid block specifier '");
    e = e + w->pValue->c_str() + "'";
