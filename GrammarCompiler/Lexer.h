...
namespace lex {
	...
	public:
		...
		Lexer( const char *fileName )	{ scanner=new Scanner(fileName); }
		void	AttachFile( const char* fileName )	{ scanner->AttachFile(fileName); }
	...
	inline bool Lexer::IsValidIdentifier( const char *id ) const
	{
		...
		do if ( !IsLetter(*id) && !IsDigit(*id) ) return false; while ( *++id );
		return true;
	}
	...
	{
		string *s = new string;
		try { s = scanner->GetLexeme(); }
		catch( ScannerException& e ) { throw LexerException( e.what() ); }
		if ( s==0 ) return 0;
